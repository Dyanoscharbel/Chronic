...
// Basic Toast implementation (replace with a proper library if needed)
const ToastContext = React.createContext<{ showToast: (message: string) => void; toasts: string[] }>({
  showToast: () => {},
  toasts: [],
});
    ...
    <ToastContext.Provider value={{ showToast, toasts }}>
      ...
      <div style={{ position: 'absolute', top: 20, right: 20 }}>
        {toasts.map((toast, index) => (
          <div key={index} style={{ backgroundColor: '#f0f0f0', padding: '10px', marginBottom: '5px', border: '1px solid #ccc' }}>
            {toast}
          </div>
        ))}
      </div>
...
interface PatientTableProps {
  ...
  error: any; // Added error handling
}

export function PatientTable({ patients, totalPatients, error }: PatientTableProps) {
